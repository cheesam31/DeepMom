...
class DeepMomApp(MDApp):
    def __init__(self, **kwargs):
        ...
        self.dialog = MDDialog(title="Setting number of epochs",
                               type="custom",
                               content_cls=EpochSetting(),
                               buttons=[MDRectangleFlatButton(text="Confirm", line_color=utils.get_color_from_hex('#158585FF'),
                                                              text_color=utils.get_color_from_hex('#158585FF'), on_release=self.button_release)])
    ...
    def on_message(self, client, userdata, msg):
        if not self.finish_flag:
            ...
            if not self._dashboard_screen.hover_graph.current_epoch:
            ...
            self._info_dict['max_acc'] = learn_curve['accuracy'] if self._info_dict['max_acc'] < learn_curve['accuracy'] else self._info_dict[
                'max_acc']
    ...
    def discard_data(self, args):
        self._dashboard_screen.ids.max_acc.text = 'Unknown'
        self._dashboard_screen.ids.min_loss.text = 'Unknown'
        self._dashboard_screen.ids.max_val_acc.text = 'Unknown'
        self._dashboard_screen.ids.min_val_loss.text = 'Unknown'
        self._dashboard_screen.ids.remain_time.text = 'Unknown'
        self._dashboard_screen.ids.progress_value.text = 'Unknown'
        self._dashboard_screen.ids.progress_epochs.text = '0 / {}'.format(self._dashboard_screen.hover_graph.epoch)
        self._dashboard_screen.ids.progress_bar.value = 0
        self.finish_flag = False

        if not self._using_validation:
            self._dashboard_screen.hover_graph.reset()
        ...
        self._dashboard_screen.hover_graph.current_epoch = 0
        self._dashboard_screen.hover_graph.accuracy_plot.points = [(0, 0)]
        self._dashboard_screen.hover_graph.val_accuracy_plot.points = [(0, 0)]
        self._dashboard_screen.hover_graph.loss_plot.points = [(0, 0)]
        self._dashboard_screen.hover_graph.val_loss_plot.points = [(0, 0)]

    def export_file(self, args):
        try:
            data_frame = pandas.DataFrame(columns=['accuracy', 'validation_accuracy', 'loss', 'validation_loss'])
            data_frame['accuracy'] = numpy.array(self._dashboard_screen.hover_graph.accuracy_plot.points)[:, 1]
            data_frame['validation_accuracy'] = numpy.array(self._dashboard_screen.hover_graph.val_accuracy_plot.points)[:, 1]
            data_frame['loss'] = numpy.array(self._dashboard_screen.hover_graph.loss_plot.points)[:, 1]
            data_frame['validation_loss'] = numpy.array(self._dashboard_screen.hover_graph.val_loss_plot.points)[:, 1]
    ...
    def button_release(self, args):
        try:
            epochs = int(self.dialog.content_cls.ids.epochs_text.text)

            if epochs < self._dashboard_screen.hover_graph.current_epoch:
                self._dashboard_screen.ids.remain_time.text = 'Finish'
                self._dashboard_screen.hover_graph.current_epoch = epochs
                self.finish_flag = True

            value = self._dashboard_screen.hover_graph.current_epoch / self._dashboard_screen.hover_graph.epoch * 100
            self._dashboard_screen.ids.progress_value.text = '[b]{}[/b]%'.format(round(value, 2))
            self._dashboard_screen.hover_graph.graph.xmax = epochs
            self._dashboard_screen.hover_graph.graph.x_ticks_major = epochs / 10
            self._dashboard_screen.hover_graph.epoch = epochs
            self._dashboard_screen.ids.progress_epochs.text = '{} / {}'.format(self._dashboard_screen.hover_graph.current_epoch, epochs)
            self.dialog.dismiss()

        except ValueError:
            warn_dialog = MDDialog(title='Input must be integer', type='alert')
            warn_dialog.open()
